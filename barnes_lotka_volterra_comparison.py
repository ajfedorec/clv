...
def plot_corr(A_stein, g_stein, B_stein, A_est, g_est, B_est, filename):
    ...
    y = m * x + b
    handle = ax[0].plot(x, y, label="R = {}".format(np.round(pearsonr(A[:, 0], A[:, 1])[0], 3)), linestyle="--",
                        color="C3")
    ...
    # x = np.linspace(-10, 10, 3)
    # y = m * x + b
    # handle = ax[1].plot(x, y, label="R = {}".format(np.round(pearsonr(B[:, 0], B[:, 1])[0], 3)), linestyle="--",
    #                     color="C3")
    ...
    x = np.linspace(-2, 9, 3)
    y = m * x + b
    handle = ax[2].plot(x, y, label="R = {}".format(np.round(pearsonr(g[:, 0], g[:, 1])[0], 3)), linestyle="--",
                        color="C3")
...
if __name__ == "__main__":
    Y = pkl.load(open("data/barnes/Y.pkl", "rb"))
    # U = pkl.load(open("data/barnes/U.pkl", "rb"))
    T = pkl.load(open("data/barnes/T.pkl", "rb"))
    ...
    A_glv, g_glv, B_glv = ridge_regression_glv(X=log_Y, U=clv.U, T=T, r_A=clv.r_A, r_g=clv.r_g, r_B=clv.r_B)

    A_rel, g_rel, B_rel = compute_relative_parameters(A_glv, g_glv, B_glv, clv.denom)
    plot_corr(A_rel, g_rel, B_rel, A_clv, g_clv, B_clv, "plots/barnes_correlation.pdf")